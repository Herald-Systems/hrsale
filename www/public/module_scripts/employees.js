$(document).ready(function () {
    const fetchEmployees = async (filters = '') => {
        return $('#xin_table').dataTable({
            "bDestroy": true,
            "ajax": {
                url: main_url + "employees/employees_list" + filters,
                type: 'GET'
            },
            "language": {
    ...
    var xin_table = fetchEmployees();

    document.getElementById('department_filter').addEventListener('change', function (ev) {
        if (!!ev.target.value)
            fetchEmployees("?department_id=" + ev.target.value);
        else
            fetchEmployees();
    });
