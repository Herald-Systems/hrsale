...
use App\Models\SuperannuationModel;
...
//        $usession = $session->get('sup_username');
    ...
    public function loans_list()
    {
        ...
        $SystemModel = new SystemModel();
        //$AssetsModel = new AssetsModel();
        $ContractModel = new LoansModel();
        $request = Services::request();
        $id = $request->uri->getSegment(4);
        $user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
        $get_data = $ContractModel->where('user_id', $id)->findAll();
        $data = array();

        foreach ($get_data as $r) {

            $edit = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="' . lang('Main.xin_edit') . '"><button type="button" class="btn icon-btn btn-sm btn-light-primary waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-field_type="loans" data-field_id="' . uencode($r['id']) . '"><i class="feather icon-edit"></i></button></span>';
            $delete = '<span data-toggle="tooltip" data-placement="top" data-state="danger" title="' . lang('Main.xin_delete') . '"><button type="button" class="btn icon-btn btn-sm btn-light-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-field_type="all_loans" data-record-id="' . uencode($r['id']) . '"><i class="feather icon-trash-2"></i></button></span>';
    ...
    public function superannuation_list()
    {
        ...
        $UsersModel = new UsersModel();
        $superannuationModel = new SuperannuationModel();
        $request = Services::request();
        $id = $request->uri->getSegment(4);
        $user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
        $get_data = $superannuationModel->where('user_id', $id)->findAll();
        $data = array();

        foreach ($get_data as $r) {

            $edit = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="' . lang('Main.xin_edit') . '"><button type="button" class="btn icon-btn btn-sm btn-light-primary waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-field_type="superannuation" data-field_id="' . uencode($r['id']) . '"><i class="feather icon-edit"></i></button></span>';
            $delete = '<span data-toggle="tooltip" data-placement="top" data-state="danger" title="' . lang('Main.xin_delete') . '"><button type="button" class="btn icon-btn btn-sm btn-light-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-field_type="all_superannuation" data-record-id="' . uencode($r['id']) . '"><i class="feather icon-trash-2"></i></button></span>';
            ...
            $data[] = array(
                $salary_option,
                number_format($r['nambawan_super'], 2),
                number_format($r['posf_super'], 2),
                number_format($r['pos_voluntary_amount']),
            );
    ...
    public function add_loan()
    {
        ...
        if (!$this->validate($rules)) {
            $ruleErrors = [
                "loan_id" => $validation->getError('loan_id'),
                "institution" => $validation->getError('institution'),
            ...
            $data = [
                'user_id' => $id,
                'loan_id' => $this->request->getPost('loan_id', FILTER_SANITIZE_STRING),
                'institution' => $this->request->getPost('institution', FILTER_SANITIZE_STRING),
                'amount' => $this->request->getPost('amount', FILTER_SANITIZE_STRING),
            ];
            $result = $loansModel->insert($data);
            $Return['csrf_hash'] = csrf_hash();
            if ($result) {
    ...
    public function add_superannuation()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $Model = new SuperannuationModel();
            $id = udecode($this->request->getPost('token', FILTER_SANITIZE_STRING));
            $data = [
                'user_id' => $id,
                'nambawan_super' => $this->request->getPost('nambawan_super', FILTER_SANITIZE_STRING),
                'posf_super' => $this->request->getPost('posf_super', FILTER_SANITIZE_STRING),
                'pos_voluntary_amount' => $this->request->getPost('pos_voluntary_amount', FILTER_SANITIZE_STRING),
            ];
    ...
    public function update_loan()
    {
        ...
        if ($this->request->getPost('type') === 'edit_record') {
            ...
            if (!$this->validate($rules)) {
                ...
                $result = $loansModel->update($id, $data);
                $Return['csrf_hash'] = csrf_hash();
                if ($result == TRUE) {
    ...
    public function update_superannuation()
    {
    ...
    public function update_basic_info()
    {
        ...
        if ($this->request->getPost('type') === 'edit_record') {
            ...
            $data = [
                ...
                'gender' => $gender,
                'is_active' => $status,
                'calculate_payroll' => $this->request->getPost('calculate_payroll', FILTER_SANITIZE_STRING),
    ...
    public function delete_all_loans()
    {
        ...
        if ($this->request->getPost('_method') == 'DELETE') {
            ...
            $Return['csrf_hash'] = csrf_hash();
            $LoansModal = new LoansModel();
            $result = $LoansModal->where('id', $id)->delete($id);
            if ($result == TRUE) {
    ...
    public function delete_all_superannuation()
    {
